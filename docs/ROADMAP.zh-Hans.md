...
- [x] 服务器实时监控(SSH 只读采集、服务器零 agent):系统信息 + CPU/内存/磁盘/负载环形仪表 +
      容器/服务/端口/进程计数 + 流量曲线,顶栏下拉「图层」面板,悬停看详情;
- [x] 测试与 CI 基线:Rust 单测/集成 + 前端 vitest + Clippy(`-D warnings`) + 一站式 `pnpm ci:check`,
      含命令风险分级、脱敏、监控解析、取消注册表、Codex 协议、前端格式化/api mock 等回归;
- [x] 对抗式 bug 猎杀一轮:修复含两个高危安全(防火墙 -s/-S 混淆、dd of= 覆盖关键文件被判 Low)在内的真实缺陷。
